...
app.all('/new/*', user.requirePermission('manage content'), function(req, res, next) {
    var ent = new document.Document;

    if (req.params[0] && ent.setType(req.params[0])) {
        // Set up document and form.
...
app.all('/edit/*', user.requirePermission('manage content'), function(req, res, next) {
    if (req.params[0]) {
        // Create document and form, define form when document loaded.
