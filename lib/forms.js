...
DocumentForm.prototype.view = view('content');
...
NewDocumentForm.on('load', function(req,res, track) {
  if (req.params[0]) {
    ...
    this.pageTitle = 'Create new ' + strip_tags(this.document.data.type);
    this.title = 'Create new ' + strip_tags(this.document.data.type);
  }
  else {
...
var EditDocumentForm = DocumentForm.create(DocumentForm);
...
EditDocumentForm.on('load', function(req,res,track) {
    ...
    this.document.load(req.params[0], track(function(err, ent) {
        ...
        req.form.pageTitle = 'Edit ' + strip_tags(req.form.document.data._id);
        req.form.title = 'Edit ' + strip_tags(req.form.document.data._id);
    }));
});
...
DeleteDocumentForm.on('load', function(req, res, track) {
   var doc = require('./document');
   this.document = new doc.Document();
   track(this.document.load(req.params[0], track(function(err, ent) {
      req.form.defaults = forms.utils.deepClone(req.form.document.data);
      req.form.pageTitle = 'Delete ' + strip_tags(req.form.document.data._id);
      req.form.title = 'Delete ' + strip_tags(req.form.document.data._id);
   })));
})
