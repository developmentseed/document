...
/**
 * Formats UNIX time to MMM DD YYYY HH:MM
 *
...
var Document = function() {
    ...
    var allowedTags = 'a|b|blockquote|code|del|dd|dl|dt|em|h1|h2|h3|' +
            ...
            'table|thead|th|tbody|tfoot|tr|td|div|span',
        allowedAttributes = {
    ...
    this.renderers = {
        ...
        'markdown': function(o) {
            return {prose: md(o, true, allowedTags, allowedAttributes)};
        },
        'title' : function(o) {
...
Document.prototype.load = function(id, callback) {
    ...
    this.db.get(id, function(err, doc) {
        if (!err) {
            ...
            that.data.timestamp = doc.timestamp || Math.round(Date.now() / 1000);
...
Document.prototype.render = function(req) {
    ...
    var locals = {
        ...
        publishDate: formatDate(new Date(this.data.timestamp * 1000)),
        content: { 
            _template: __dirname + '/document',
            items: output 
        },
        suppressTitle: true,
    };
...
function documentLoader(req, res, next) {
    ...
    ent.load(req.params[0], function(err, ent) {
        ...
        ent.db.view('pages/hierarchy', {
            ...
            ent.tree.children = _(results).chain()
                ...
                .sortBy(function(v) { return parseInt(v.weight || 3); })
                .value();
